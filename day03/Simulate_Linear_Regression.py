...
def train(x,y,coef):
    ...
    #7. print training result：
    print(f'{epochs} round of average loss is: {loss_list}')
    ...
    #9. Plot the repationship between predict value and true value
    #9.1 Plot the distribution of sample points
    plt.scatter(x,y)
    #9.2 Plot predict value
    y_pred = torch.tensor(data=[v * model.weight + model.bias for v in x])
    #9.3 Find true value
    y_true = torch.tensor(data= [v * coef + 14.5 for v in x])
    #9,4 Plot line chart for true and predict value
    plt.plot(x, y_pred, color = 'red', label = 'predict value')
    plt.plot(x, y_true, color = 'green', label = 'true value')
    plt.legend()
    plt.grid()
    plt.show()
